...
def test_tool_calls(server):
    ...
    assert "q_norm" in _text(r) and "## Takeaways" in _text(r)
    r = asyncio.run(server.call_tool("plan_serving", {"model_id": "Qwen/Qwen3-8B", "gpu_memory_gb": 16, "weights": "int4"}))
    assert '"fits": true' in _text(r) and "quantized to int4" in _text(r)
