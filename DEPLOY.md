...
| `MODELMAP_HF_TOKEN` | unset | server-side token for Hub *metadata* calls (search, trending, config/header reads): authenticated requests carry a much higher rate limit than a shared egress IP, which matters once real traffic arrives. Use a **fine-grained read token with access to no repos**. Caveat: any gated repo the token's account has accepted becomes mappable by every visitor — that is why this is a separate, explicit opt-in and the generic `HF_TOKEN` is never read |
